...
# Load JSON data
with Path(BASE_DIR/"gonne.json").open(encoding="utf-8") as f:
    data = json.load(f)
...
if "social_links" in data:
    for link in data["social_links"]:
        if link.get("svg_path"):
            with Path(BASE_DIR/link["svg_path"]).open(encoding="utf-8") as svg_file:
...
with Path(BASE_DIR/"index.html").open("w", encoding="utf-8") as f:
    f.write(html_output)

with Path(BASE_DIR/"resume.html").open("w", encoding="utf-8") as f:
